...
st.markdown("""
    ...
    .secondary-btn > button { background-color: white; color: #666; border: 1px solid #ccc; height: 40px !important; }
    </style>
...
if st.session_state.app_step == 1:
    ...
    st.write("초기 직장인 가입자의 51.3%가 이미 경험 중입니다.") [cite: 96]
    st.info("📌 **핵심 가치**\n- 업무 시간 20% 단축 증명 [cite: 154]\n- AI 일정 관리 및 실시간 협업")
...
# 2단계: 요금제 선택 (이탈률 45% 개선 전략)
    ...
    st.markdown("<p style='font-size:0.9em;'>지금 시작하면 <b>7일간 모든 기능을 무료</b>로 체험할 수 있습니다.</p>", unsafe_allow_html=True) [cite: 91]

    # Free 플랜 (대학생 이탈 방지) [cite: 162]
    ...
    # Starter Pro (직장인 타겟 핵심 플랜) [cite: 158]
...
# 3단계: 회원가입 (71.9% 이탈 해결 UI)
    ...
    st.markdown("<div style='text-align:center;'><h4 style='color:#0056b3;'>오늘부터 칼퇴를 시작하세요.</h4><p style='font-size:0.8em; color:#666;'>1분이면 모든 준비가 끝납니다.</p></div>", unsafe_allow_html=True) [cite: 186]
...
# 4단계: 완료
    ...
    st.write("메일함으로 전송된 **'칼퇴 치트키 템플릿'**을 확인해 보세요.") [cite: 191]
